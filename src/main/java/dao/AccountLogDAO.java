...
public class AccountLogDAO {
	...
	public void add(AccountLog accountLog) {
		...
		memberMapper.update(member);//标记提交缴费
		...
		if(memberAlog <= 0) {
			...
			//判断之前的缴费是否为空,之前出现过错误,所以加了个判断
			if(accountlogOld==null) {
				accountLogMapper.add(accountLog);
				return;
			}else {
				if(accountlogOld.getDate()==null) {
			...
			if (DateUtils.isSameDay(accountlogOld.getDate(), accountLog.getDate())) {//如果是同一天缴费  金币相加
