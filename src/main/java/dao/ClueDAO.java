...
public class ClueDAO {
	...
	public Result getClues(Integer type, Integer aid, String qq, int page, int rows) {
		...
		if(type!=4){
			list = clueMapper.getClues(type, aid, qq);
		}else{
			list=clueMapper.getRecent();
		}
	...
	public Clue checkValid(Clue clue){
		
		return clueMapper.checkValid(clue);
		
	}
	public Clue checkValqq(Clue clue){
		
		return clueMapper.checkValqq(clue);
		
	}
	
	public Clue getClueAdminById(int id){
		return clueMapper.getClueAdminById(id);
	}
}
