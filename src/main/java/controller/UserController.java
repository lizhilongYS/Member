...
//		System.out.println(userDTO);
	...
	public String download(HttpServletResponse response, HttpServletRequest request, int type,
		...
		String contextpath = request.getContextPath() + "//" + fileName + ".xls";
	 ...
	 * 
	 * 修改： 判断用户密码是否为12345678 如果是则强制用户修改密码  作者：罗长城 日期：2017-9-15
	 */
	...
	public String check(HttpSession session, User user, HttpServletRequest request) throws Exception {
		...
		this.logger("check.action", session);
		...
		//前端传过来的登录密码
		String pwd=MD5SaltUtils.encode(password, esalt);
		if(experience.getNum().startsWith("TY")){
		experience.setPassword(pwd);
...
*/			//存会员最后一篇周报的id
		...
		if(num.startsWith("XS")){
			Clue clue= new Clue();
			clue.setNum(num);
			clue.setPhone(password);
			clue= clueDAO.checkValid(clue);
			if(clue!=null){
				session.setAttribute("clue", clue);
				session.setAttribute("CLUE", clue);
				session.setAttribute("TURE", clue);
				return "3";
			}
		}
...
//			System.out.println(count+")))))))))))))))))))))))))");
	...
	/*
	 * 作者:农大辉
	 *  作用:写一个方法在线显示PDF协议，动态生成PDF，服务器一共有三个basic文件：协议模板，
	 * 替换参数后的协议文档，及协议PDF文件
...
//		String lasthtmlPath = generatePDF.replaceKeyWords(user, inputHTMLFileName, htmlPath, pdfPath, userInfo);
	...
	public String dlWord(HttpServletRequest request, HttpSession session, int id) throws Exception {
		...
		boolean flag = member.getStudent();
		if (flag) {
			inputDocFileName = request.getSession().getServletContext().getRealPath("") + "contract" + File.separator + "doc"
		...
		// 产生随机数，文件的名字
		fileName = String.valueOf((int) (1000000000 + Math.random() * 999999999));
		docPath = request.getSession().getServletContext().getRealPath("") + "contract" + File.separator + "temp"
	...
	public String checkUserSession(HttpSession session ) {
		return VerifyIdentity.verifyAll(session) != null ? "1" : "0";
	}
	/*
	...
	@RequestMapping("/logger.action")
	@ResponseBody
	public String logger(String content, HttpSession session){
		...
		try {
			...
			if(admin != null ){
				number = admin.getName();			
			}else if(user != null){ 
				number = user.getName();
			}else if(experience != null){
				number = experience.getNum();			
			}else if (clue != null){
				number = clue.getNum();
			}else{
				number= "no name";
		...
		String fullContent = "number:"+number+",content:"+content;
		logger.error(fullContent);
		return "1";
	}
