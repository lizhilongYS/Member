...
//	            is.close(); 
	...
	public String flag(int id,HttpServletRequest request,HttpSession session) throws IOException{		
		 try {
				jedisClient.del(TOUXIANG+pictureDAO.getPicidByid(pictureDAO.getOne(id).getUid()).getId());
				jedisClient.del("PIC");
			} catch (Exception e) {
		} 
		// System.out.println("id是============="+pictureDAO.getPicidByid(id).getId());
	...
	private List<Picture> pages(int pageno, HttpSession session, List<Picture> picture) {
	 ...
	 public String  getpic(int id,HttpServletRequest request,HttpServletResponse response) throws IOException{
		...
		public String getAllUserCoverByPage(HttpSession session,HttpServletRequest request,PictureDTO pictureDTO,int page2){
			...
			newpage=page2;	
			try {
				String string = jedisClient.hget("PIC", newpage+"");
						//hget("PIC",newpage);
				if(string!=null) {
					return string;
				}
...
//			}
			...
			json.put("returnMap", returnMap);
			try {
				jedisClient.hset("PIC", newpage+"", json.toString());
				//set("PIC"+newpage, json.toString());
				//jedisClient.expire("PIC",newpage+"", 60*60*24*3);
			} catch (Exception e) {
				// TODO: handle exception
			}
			return json.toString();
...
//			}
		...
		public void  getPictureByid(int id,HttpServletRequest request,HttpServletResponse response) throws IOException{
				try {
					String string = jedisClient.get(TOUXIANG+id);
					if(string!=null) {
						Picture picture = JsonUtils.jsonToPojo(string, Picture.class);
						if(picture==null) { 
							return;
						}
						byte[] photo = picture.getPhoto();
					...
					System.out.println("第一次保存图片,redis中没有照片");
				}
			Picture picture = pictureDAO.getOne(id);
			try {
				jedisClient.del(TOUXIANG+id);
				jedisClient.set( TOUXIANG+id, JsonUtils.objectToJson(picture));
				jedisClient.expire(TOUXIANG+id, 60*60*24*7);
			} catch (Exception e) {
				System.out.println("redis中保存图片报错");
			}
