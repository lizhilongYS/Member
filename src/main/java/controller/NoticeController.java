...
public class NoticeController {
	...
	*/
	...
	{
		try {
			jedisClient.del("GONGGAO");
		} catch (Exception e) {
			System.out.println("报错公告报错");
		}
	...
	*/
	...
	public String notice(HttpSession session) throws ParseException{
		User user=(User) session.getAttribute("TURE");
	...
	*/
	...
	public String getNotices(HttpSession session,HttpServletRequest request, NoticeDTO noticeDTO,int page2 ) throws Exception {
		...
		newpage = page2;
		try {
			String string = jedisClient.hget("GONGGAO",newpage+"");
			if(string!=null) {
				return string;
			}
		} catch (Exception e) {
			System.out.println("redis中没有公告信息");
		}
		//为了以后需求更改 这里用list接收
		...
		try {
			jedisClient.hset("GONGGAO", newpage+"",json.toString());
		} catch (Exception e) {
			System.out.println("报错公告报错");
		}
