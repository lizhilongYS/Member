...
public class AdminController {
	...
	*/
	...
	public String getAllAdmin(HttpSession session)
	{
		...
		for(int i=0; i<size; i++ )
		{
			...
			}else if(admin.getAuthority().equals("1")){
				str.append("\" class=\"chmod label label-info radius\">超");
			}else {
	...
	*/
	...
	public String chmod(Admin admin)
	{
		...
		}else if(admin.getAuthority().equals("2")){
			admin.setAuthority("0");
		}else {//对于Authority为0和2以为的直接按原值返回，不操作数据库
	...
	public List<Admin> getWorkAdmin() {
  	...
  	/*
	 * 新增
	 * 内容:访问费用相关页面前先判断是否超级管理员，如果不是则返回起始页
	 * 作者：吴文波
	 * 日期：2017-11-4
	 */
	@RequestMapping("/feedetailTwo.action")
	public String getfeedetailTwo(HttpSession session){
		Admin adminInfo = (Admin) session.getAttribute("admin");
		if (adminInfo.getAuthority().equals("1")){
			return "feedetailTwo";
		}
	 ...
	 * 日期：2017-11-4
	 */
	@RequestMapping("/feepay.action")
	public String getfeepay(HttpSession session){
		Admin adminInfo = (Admin) session.getAttribute("admin");
		if (adminInfo.getAuthority().equals("1")){
			return "feepay";
		}
	 ...
	 * 日期：2017-11-4
	 */
	@RequestMapping("/fee.action")
	public String getfee(HttpSession session){
		Admin adminInfo = (Admin) session.getAttribute("admin");
		if (adminInfo.getAuthority().equals("1")){
			return "fee";
		}
