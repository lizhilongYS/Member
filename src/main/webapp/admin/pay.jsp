...
<script type="text/javascript">
	...
	$("#tabsC").on('click','.setMember',function(){
		...
		$("#iname").val(textOnly);
		name=textOnly;
		$.post("${pageContext.request.contextPath}/member/addAttribute.action",{name:name},function(data){
			$("#timei").html(data);
		})
		$.post("${pageContext.request.contextPath}/member/getIdByName.action",{name:name},function(data){
			$("#mid").val(data);
		})
		showData();
	})
	...
	$("#go").click(function() {
		...
		//$("#period").load("${pageContext.request.contextPath}/admin/pay.jsp");
		show();
		...
		<table style="width: 800px; height: 300px">
			...
			<tr>
				<td align="left" valign="top"><c:choose>
						<c:when
							test="${myuser.member.fee && myuser.member.restInterest>0}">
						...
						每月协议交费日期：<b><mark>
									<font id=timei />
									<%-- <fmt:formatDate id="time" value="${myuser.member.time}" pattern="dd日" /> --%>
									<br />
								</mark></b>
				    	...
				    	<input type="text" id="date" placeholder="缴费日期" value="${time}">
							<br />
							<input type="button" id="important" value="重要提示"
								class="btn btn-danger radius">
							<input type="button" value="缴费预览" id="preview"
								class="btn btn-secondary radius">
							<input type="button" value="确认缴费" id="go"
								class="btn btn-primary radius">
							<div id="infoshow" style="color: red;">如果上传凭证,管理员审核会更快呦。</div>
					 	备注：利息在所有本金交清后结算，后面的费用比约定早交可以抵消利息！！！！
					</c:otherwise>
					</c:choose></td>
				<td align="left" valign="top" style="width: 400px"><input
					id="file" type="file" onchange="photo(this,'photo')"
					class="form-control" style="display: none"> <a
					class="btn btn-primary radius" onclick="$('#file').click();"><i
