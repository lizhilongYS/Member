	...
	<table class="table table-bordered" id="logs">
		...
		<c:forEach var="log" items="${LIST}" varStatus="status">
			<tr>
				...
				<td><a href='javascript:void(0)' class='forDetail'
					lang="${log.id}">查看详情</a></td>
				<td><a
					href='${pageContext.request.contextPath}/ondutylog/modifyThis.action?id=${log.id}'>修改</a></td>
			</tr>
		</c:forEach>
	</table>
	<input type="hidden" id="page" value="${PAGE}"> ${NAVBAR}
</body>
</html>