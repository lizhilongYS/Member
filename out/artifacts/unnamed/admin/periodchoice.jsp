...
#tabs a {
	...
	background: url('/Member/images/bg_ch.gif') repeat-x scroll 0% 0%
		transparent;
}
...
$(function(){
	...
	$("#saveit").click(function(){
		...
		index = layer.load(2);
		$.post("${pageContext.request.contextPath}/member/isNullUserinfo.action",{id:id},function(data){
			     layer.closeAll();
			     if(data)
				   {
			    	 layer.msg('新增会员还没有填写基本信息,审核前请通知新增会员登录官网填写基本信息！', {
						    icon: 1,
						    time: 4000
						});
				       return false;
	...
	$("#tabs a").each(function(){
		...
		$(this).click(function(){
			...
			if(ref == '#tab4') {
				var id=${param.id};
				//下面的代码一定要省略掉,因为当点击自定义按钮后再点击新分期的时候会显示不出数据
				//$("#perpiodtb").html("");
				$("#period").html("");
				$("#hide").show();
				$("#saveit").unbind('click').click(function(){
...
<body>
	<div id="tab">
		<span id="tabs"> <a href="javascript:void(0);" ref="#tab1"
			class="here">新分期</a> <a href="javascript:void(0);" ref="#tab2"
			class="nor">旧分期</a> <a href="javascript:void(0);" ref="#tab3"
			class="nor">全费</a> <a href="javascript:void(0);" ref="#tab4"
			class="custom">自定义</a>
		</span>
	</div>
	...
	<br />
	<div id="title">
		姓名：<span id="name"></span>&nbsp;&nbsp;&nbsp;&nbsp; 类型：<span
			id="studentType"></span>&nbsp;&nbsp;&nbsp;&nbsp; 注册日期:<span
			id="registerTime"></span>
	</div>
	<br />
	<div id="perpiodtb">
		<table>
			<tr>
				<td>本金合计：<input type="text" id="totalamount" value="8000"
					style="width: 60px;" /></td>
				<td>首付：<input type="text" id="firstamount" value="1500"
					style="width: 60px;" /></td>
				<td>月利率：<input type="text" id="interest" value="0.02"
					style="width: 60px;" /></td>
				<td>月供：<input type="text" id="monthpay" value="500"
					style="width: 60px;" /></td>
				<td><input type="button" id="calculator" value="显示还款详单"></td>
			</tr>
			<tr>
				<td>实交合计：<input type="text" id="amount" readonly
					style="width: 60px;" /></td>
				<td>利息合计：<input type="text" id="interests" readonly
					style="width: 60px;" /></td>
			</tr>
		</table>
	</div>
	...
	<br />
	<div id="btn">
		<input type="button" id="saveit" value="通过审核">
	</div>
</body>
</html>