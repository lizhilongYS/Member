...
$(function(){
	...
	$("#add").unbind("click").click(function(){
		layer.open({
			  ...
			  shift: 1,
			  shade: 0.5, //开启遮罩关闭
			  content: '${pageContext.request.contextPath}/admin/companyaccount/add.jsp?ftype=1',
			  end: function(){
				getData();
			  }
		});
	...
	function getData(){
		//设置同步
		$.ajaxSetup({async:false});
		var json = {"admin.id":$("[name='assistant']").val(),ftype:1,"page.currentPage":page};
		//查看 北京地区 或者重庆地区按钮是否被选中
		$(".butname").each(function(i,e){
			if($(this).hasClass("active") && (this.lang != "0")){
				json = {"admin.id":$("[name='assistant']").val(),ftype:1,"province.id":this.lang,"page.currentPage":page}
			}
		})
	...
	function drawTable(data){
		...
		for(i in data){
			...
			line += "<td><a herf='#' class='flag' lang='"+data[i].id+"'>";
	...
	function modify(){
		$("#tbody").on('click','.mod',function(){
			...
			//原来根据cauthority判断改成根据authority判断
			...
			layer.open({
				  ...
				  shift: 1,
				  shade: 0.5, //开启遮罩关闭
				  content: '${pageContext.request.contextPath}/admin/companyaccount/modify.jsp?id='+caid+"&ftype=1",
				  end: function(){
					  //location.reload();
					  getData();
				  }
	...
	function deleteNode(){
		$("#tbody").on('click','.icon-delete-middle',function(){
			var caid  = this.lang.split(",")[0];
			var aid  = this.lang.split(",")[1];
			//解决权限问题 ,原来根据cauthority判断改成根据authority判断
			if("${admin.authority}" != "1"){
				//如果不是本人操作
				if(aid != "${admin.id}"){
