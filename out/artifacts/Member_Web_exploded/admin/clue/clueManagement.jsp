			...
			for (i = 0; i < data.length; i++) {
				...
				//判断是否有助教
				 if(data[i].admin==null){
					...
					adminName=data[i].admin.realname}; 
				//判断是否有沟通
				 if(data[i].communication==null){
					 communicationData="";
				 }else{
					 communicationData=data[i].communication.time;
				 }
				line += "<td>" + (i + 1) + "</td>";
				...
				if(type==4){
				line += "<td>" + datePattern(communicationData) + "</td>";
					}
				line +="<td>" + isBlank(data[i].counts, 0) + "次</td>";
				line += "<td>" + isBlank(data[i].exnum) + "</td>";
				line += "<td class='aname' lang='"+data[i].id+","+data[i].qq+"'>" +adminName + "</td>";
		...
		function addCommunicationclick() {
			$(".addCommunication").click(function() {
				...
				layer.open({
					...
					shift : 1,
					shade : 0.5, //开启遮罩关闭
					content : '${pageContext.request.contextPath}/admin/clue/addCommunication.jsp?id='+ str[0] + '&name=' + str[2],							
 					end : function() {
			...
			$(".lookCommunication").click(function() {
				...
				layer.open({
					...
					content : '${pageContext.request.contextPath}/admin/clue/lookCommunication.jsp?id='
						...
						if($("#flag").val() >= 1) {
							if(type==4){
								$(tr_element).eq(7).html((parseInt($(tr_element).eq(7).html()) - $("#flag").val()) + "次");
							}else{
						...
						if($("#flag").val() == "1") {
							$.post("${pageContext.request.contextPath}/clue/getClueById.action", {id : str[0]}, function(data) {
								if (isDataStatus(data)) {
									...
									$(tr_element).eq(3).html(data.data.admin.realname);
									getData(type, aid, 1, qq);
		...
		<div class="panel-header">
			...
			<div class="form-group">
				<span class="select-box radius"
					style="width: 100px; background: white;"> <select
					class="select" id="aid">
						<option value="0">所有人</option>
						<option value="73">刘永</option>
						<option value="74">张海浪</option>
						<option value="75">李子月</option>
						<option value="76">刘申鹏</option>
						<option value="77">王健</option>
						<option value="79">龚道松</option>
						<option value="80">周兴华</option>
				</select>
				</span> <input id="qq" type="text" placeholder="QQ号" style="width: 100px"
